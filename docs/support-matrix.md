...
| GitHub Packages registry | Alias publisher configured | GitHub Packages publishes the same SDK contents as `@JustineDevs/agents`; npm remains canonical as `@jstn-sdk/agents` |
...
GitHub Releases are synchronized by semantic-release. GitHub Packages uses the
owner-compatible alias `@JustineDevs/agents`; npm remains the canonical package
at `@jstn-sdk/agents`. The `github-package.yml` workflow publishes the alias
with the repository `GITHUB_TOKEN` and keeps both distribution channels
available without changing the npm contract.
